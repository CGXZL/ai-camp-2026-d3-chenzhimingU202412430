...
def verify_real_data(source: Path) -> dict[str, object]:
    """Verify the exact real Household Power Consumption file before modeling."""
    ...
    if len(raw) != EXPECTED_ROWS:
        raise ValueError(
            f"Expected {EXPECTED_ROWS} minute rows but found {len(raw)}"
        )
    ...
    if timestamp.min().year != 2006 or timestamp.max().year != 2010:
        raise ValueError(
            f"Unexpected time range: {timestamp.min()} to {timestamp.max()}"
        )
    return {
        "rows": len(raw),
        "columns": len(raw.columns),
        "time_range": [str(timestamp.min()), str(timestamp.max())],
        "missing_global_active_power": int(
            raw["Global_active_power"].isna().sum()
        ),
    }
